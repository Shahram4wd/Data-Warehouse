...
class SalesRabbitLeadSyncEngine(SalesRabbitBaseSyncEngine):
    ...
    async def _fetch_leads_batches_since(self, since_date: datetime, batch_size: int, max_records: int) -> AsyncGenerator[List[Dict], None]:
        """Fetch leads since date in batches using pagination"""
        page = 1
        total_fetched = 0
        ...
        while True:
            try:
                ...
                total_fetched += len(batch)
                logger.info(f"Fetched page {page} with {len(batch)} records since {since_date} (total: {total_fetched})")
                ...
                logger.error(f"Error fetching page {page} since {since_date}: {e}")
                break
