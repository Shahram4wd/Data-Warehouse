"""
Base CallRail API client following CRM sync guide architecture
"""
import logging
import asyncio
import os
...
class CallRailBaseClient(BaseAPIClient):
    """Base CallRail API client with common functionality following CRM sync guide"""
    
    def __init__(self, api_token=None):
        super().__init__(base_url="https://api.callrail.com/v3", timeout=60)
        # First try passed token, then Django settings, then environment
        self.api_token = (
            api_token or 
            getattr(settings, 'CALLRAIL_API_KEY', None) or
            os.getenv('CALLRAIL_API_KEY')
        )
    ...
    async def authenticate(self) -> None:
        ...
        logger.info(f"CallRail client initialized with token: {self.api_token[:5]}...{self.api_token[-5:]}")
    
    def get_rate_limit_headers(self) -> Dict[str, str]:
        """Return rate limit headers for CallRail API"""
        return {
    ...
    async def make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if "HTTP 429" in error_str or "rate limit" in error_str.lower():
                ...
                raise RateLimitException("CallRail API rate limit exceeded")
