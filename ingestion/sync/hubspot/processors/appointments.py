    ...
    def validate_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if record.get('hs_object_id'):
            try:
                ...
                logger.warning(f"Invalid HubSpot object ID '{record['hs_object_id']}' for appointment {record_id}: {e}")
        ...
        if record.get('phone1'):
            try:
                ...
                logger.warning(f"Invalid phone1 '{record['phone1']}' for appointment {record_id}: {e}")
                
        if record.get('phone2'):
            try:
                record['phone2'] = self.validate_field('phone2', record['phone2'], 'phone', record)
            except ValidationException as e:
                logger.warning(f"Invalid phone2 '{record['phone2']}' for appointment {record_id}: {e}")
        ...
        if record.get('email'):
            try:
                ...
                logger.warning(f"Invalid email '{record['email']}' for appointment {record_id}: {e}")
        ...
        if record.get('canvasser_email'):
            try:
                ...
                logger.warning(f"Invalid canvasser_email '{record['canvasser_email']}' for appointment {record_id}: {e}")
        ...
        if record.get('zip'):
            try:
                ...
                logger.warning(f"Invalid zip code '{record['zip']}' for appointment {record_id}: {e}")
        ...
        if record.get('state'):
            try:
                ...
                logger.warning(f"Invalid state code '{record['state']}' for appointment {record_id}: {e}")
                # Keep original value if validation fails
        ...
        for field in datetime_fields:
            if record.get(field):
                try:
                    ...
                    logger.warning(f"Using legacy datetime parsing for field '{field}' with value '{record[field]}' for appointment {record_id}: {e}")
        ...
        for field in boolean_fields:
            if record.get(field) is not None:
                try:
                    ...
                    # Use legacy parsing as fallback
                    logger.warning(f"Using legacy boolean parsing for field '{field}' with value '{record[field]}' for appointment {record_id}: {e}")
        ...
        if 'is_complete' in record:
            try:
                ...
                # Use legacy parsing as fallback
                logger.warning(f"Using legacy boolean parsing for is_complete with value '{record['is_complete']}' for appointment {record_id}: {e}")
