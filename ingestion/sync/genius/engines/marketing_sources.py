...
class GeniusMarketingSourcesSyncEngine(GeniusBaseSyncEngine):
    ...
    def sync_marketing_sources(self, 
        ...
        try:
            return asyncio.run(self.sync_marketing_sources_async(**sync_params))
        except RuntimeError as e:
            if "asyncio.run() cannot be called from a running event loop" in str(e):
                # Already in an event loop, use different approach
                import asyncio
                loop = asyncio.get_event_loop()
                return loop.run_until_complete(self.sync_marketing_sources_async(**sync_params))
            else:
                raise
    ...
    async def execute_sync(self, 
        ...
        return await self.sync_marketing_sources_async(
