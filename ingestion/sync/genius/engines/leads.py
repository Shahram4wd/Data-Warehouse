"""
Leads sync engine for Genius CRM data synchronization
"""
import logging
...
class GeniusLeadsSyncEngine:
    """Sync engine for Genius leads data with chunked processing"""
    ...
    def sync_leads(self, since_date: Optional[datetime] = None, 
                  force_overwrite: bool = False, 
                  dry_run: bool = False,
                  max_records: Optional[int] = None) -> Dict[str, Any]:
        """
        Sync leads data with chunked processing
        ...
        try:
            ...
            if max_records and max_records <= 10000:
                ...
                leads_data = self.client.get_leads(since_date=since_date, limit=max_records)
                stats = self._process_leads_batch(leads_data, force_overwrite, dry_run, stats)
            else:
                # For larger datasets, use chunked processing
                stats = self._sync_chunked_leads(since_date, force_overwrite, dry_run, max_records, stats)
    ...
    def _sync_chunked_leads(self, since_date: Optional[datetime], force_overwrite: bool, 
                           dry_run: bool, max_records: Optional[int], 
                           stats: Dict[str, Any]) -> Dict[str, Any]:
        """Process leads data in chunks for large datasets"""
        ...
        while True:
            ...
            # Fetch chunk data
            chunk_data = self.client.get_chunked_leads(offset, current_chunk_size, since_date)
            ...
            # Process this chunk
            chunk_stats = self._process_leads_batch(chunk_data, force_overwrite, dry_run, stats)
    ...
    def _process_leads_batch(self, leads_data: list, force_overwrite: bool, 
        ...
        batch_count = (len(leads_data) + self.batch_size - 1) // self.batch_size
        logger.info(f"Processing {len(leads_data)} leads in {batch_count} batches of {self.batch_size}")
        
        for i in range(0, len(leads_data), self.batch_size):
            batch_num = (i // self.batch_size) + 1
            batch_data = leads_data[i:i + self.batch_size]
            
            logger.info(f"Processing batch {batch_num}/{batch_count}: records {i+1}-{min(i+self.batch_size, len(leads_data))}")
