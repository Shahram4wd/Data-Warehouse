"""
Genius User Associations Sync Engine
"""
import asyncio
import logging
...
class GeniusUserAssociationsSyncEngine(GeniusBaseSyncEngine):
    """Sync engine for Genius user associations data"""
    
    def __init__(self):
        super().__init__(entity_type="user_associations")
        self.sync_type = "user_associations"
        self.client = GeniusUserAssociationsClient()
        self.processor = GeniusUserAssociationsProcessor()
    ...
    async def execute_sync(self, 
        ...
        """Execute the user associations sync process"""
        ...
        try:
            ...
            logger.info(f"Starting Genius user associations sync with strategy: {sync_strategy}")
            if dry_run:
            ...
            logger.info(f"Completed Genius user associations sync: {total_stats['created']} created, "
            ...
            # Mark sync as failed
            error_msg = f"Genius user associations sync failed: {str(e)}"
