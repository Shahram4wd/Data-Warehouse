"""
User associations sync engine for Genius CRM data synchronization
"""
import logging
...
from ..clients.user_associations import GeniusUserAssociationsClient  
from ..processors.user_associations import GeniusUserAssociationsProcessor
from ingestion.models.common import SyncHistory

logger = logging.getLogger(__name__)

class GeniusUserAssociationsSyncEngine:
    """Sync engine for Genius user associations data with chunked processing"""
    
    def __init__(self):
        # Import model here to avoid circular imports
        from ingestion.models import Genius_UserAssociation
        
        self.client = GeniusUserAssociationsClient()
        self.processor = GeniusUserAssociationsProcessor(Genius_UserAssociation)
        ...
        self.entity_type = 'user_associations'
    ...
    def sync_user_associations(self, since_date: Optional[datetime] = None, 
                              force_overwrite: bool = False, 
                              dry_run: bool = False,
                              max_records: Optional[int] = None) -> Dict[str, Any]:
        """
        Sync user associations data with chunked processing
        ...
        """
        logger.info(f"Starting user associations sync - since_date: {since_date}, force_overwrite: {force_overwrite}, "
        ...
        try:
            ...
            if max_records and max_records <= 10000:
                ...
                user_associations_data = self.client.get_user_associations(since_date=since_date, limit=max_records)
                stats = self._process_user_associations_batch(user_associations_data, force_overwrite, dry_run, stats)
            else:
                # For larger datasets, use chunked processing
                stats = self._sync_chunked_user_associations(since_date, force_overwrite, dry_run, max_records, stats)
            
            logger.info(f"User associations sync completed - Stats: {stats}")
    ...
    def _sync_chunked_user_associations(self, since_date: Optional[datetime], force_overwrite: bool, 
                                       dry_run: bool, max_records: Optional[int], 
                                       stats: Dict[str, Any]) -> Dict[str, Any]:
        """Process user associations data in chunks for large datasets"""
        ...
        while True:
            ...
            # Fetch chunk data
            chunk_data = self.client.get_chunked_user_associations(offset, current_chunk_size, since_date)
            ...
            # Process this chunk
            chunk_stats = self._process_user_associations_batch(chunk_data, force_overwrite, dry_run, stats)
    ...
    def _process_user_associations_batch(self, user_associations_data: list, force_overwrite: bool, 
                                        dry_run: bool, stats: Dict[str, Any]) -> Dict[str, Any]:
        """Process a batch of user associations data using bulk operations"""
        
        if not user_associations_data:
            return stats
            
        field_mapping = self.client.get_field_mapping()
        
        # Process in smaller batches for efficiency
        batch_count = (len(user_associations_data) + self.batch_size - 1) // self.batch_size
        logger.info(f"Processing {len(user_associations_data)} user associations in {batch_count} batches of {self.batch_size}")
        
        for i in range(0, len(user_associations_data), self.batch_size):
            batch_num = (i // self.batch_size) + 1
            batch_data = user_associations_data[i:i + self.batch_size]
            
            logger.info(f"Processing batch {batch_num}/{batch_count}: records {i+1}-{min(i+self.batch_size, len(user_associations_data))}")
