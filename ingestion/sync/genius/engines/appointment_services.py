"""
Appointment Services sync engine for Genius CRM data synchronization
"""
import logging
...
from ..clients.appointment_services import GeniusAppointmentServicesClient  
from ..processors.appointment_services import GeniusAppointmentServicesProcessor
from ingestion.models import Genius_AppointmentService
from ingestion.models.common import SyncHistory

logger = logging.getLogger(__name__)

class GeniusAppointmentServicesSyncEngine:
    """Sync engine for Genius appointment services data with chunked processing"""
    
    def __init__(self):
        self.client = GeniusAppointmentServicesClient()
        self.processor = GeniusAppointmentServicesProcessor(Genius_AppointmentService)
        ...
        self.entity_type = 'appointment_services'
    ...
    async def execute_sync(self, 
                          ...
                          debug: bool = False,
                          **kwargs) -> Dict[str, Any]:
        """Execute sync operation (async interface for compatibility)"""
        
        # Determine sync date - use provided since/start_date or get from last sync
        sync_since = None
        if not full:
            sync_since = since or start_date or self.get_last_sync_timestamp()
        
        logger.info(f"Starting {self.entity_type} sync (full={full}, dry_run={dry_run}, since={sync_since})")
        ...
        try:
            # Call the synchronous sync method
            result = self.sync_appointment_services(
    ...
    def sync_appointment_services(self, since_date: Optional[datetime] = None, 
                                force_overwrite: bool = False, 
                                dry_run: bool = False,
                                max_records: Optional[int] = None) -> Dict[str, Any]:
        """
        Sync appointment services data with chunked processing
        ...
        try:
            ...
            logger.info(f"Total appointment services to process: {total_count:,}")
            ...
            if max_records and max_records <= 10000:
                ...
                appointment_services_data = self.client.get_appointment_services(since_date=since_date, limit=max_records)
                stats = self._process_appointment_services_batch(appointment_services_data, force_overwrite, dry_run, stats)
            else:
    ...
    def _process_appointment_services_batch(self, appointment_services_data: list, force_overwrite: bool, 
        ...
        """Process a batch of appointment services data using bulk operations"""
        
        if not appointment_services_data:
            return stats
            
        field_mapping = self.client.get_field_mapping()
        
        # Process in smaller batches for efficiency
        batch_count = (len(appointment_services_data) + self.batch_size - 1) // self.batch_size
        logger.info(f"Processing {len(appointment_services_data)} appointment services in {batch_count} batches of {self.batch_size}")
        
        for i in range(0, len(appointment_services_data), self.batch_size):
            batch_num = (i // self.batch_size) + 1
            batch_data = appointment_services_data[i:i + self.batch_size]
            
            logger.info(f"Processing batch {batch_num}/{batch_count}: records {i+1}-{min(i+self.batch_size, len(appointment_services_data))}")
