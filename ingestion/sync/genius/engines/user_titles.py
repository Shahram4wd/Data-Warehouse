"""
User titles sync engine for Genius CRM data synchronization
"""
import logging
...
from ..clients.user_titles import GeniusUserTitlesClient  
from ..processors.user_titles import GeniusUserTitlesProcessor
from ingestion.models.common import SyncHistory

logger = logging.getLogger(__name__)

class GeniusUserTitlesSyncEngine:
    """Sync engine for Genius user titles data with chunked processing"""
    
    def __init__(self):
        # Import model here to avoid circular imports
        from ingestion.models import Genius_UserTitle
        
        self.client = GeniusUserTitlesClient()
        self.processor = GeniusUserTitlesProcessor(Genius_UserTitle)
    ...
    def sync_user_titles(self, since_date: Optional[datetime] = None, 
                        force_overwrite: bool = False, 
                        dry_run: bool = False,
                        max_records: Optional[int] = None) -> Dict[str, Any]:
        """
        Sync user titles data with chunked processing
        ...
        """
        logger.info(f"Starting user titles sync - since_date: {since_date}, force_overwrite: {force_overwrite}, "
        ...
        try:
            ...
            if max_records and max_records <= 10000:
                ...
                user_titles_data = self.client.get_user_titles(since_date=since_date, limit=max_records)
                stats = self._process_user_titles_batch(user_titles_data, force_overwrite, dry_run, stats)
            else:
                # For larger datasets, use chunked processing
                stats = self._sync_chunked_user_titles(since_date, force_overwrite, dry_run, max_records, stats)
            
            logger.info(f"User titles sync completed - Stats: {stats}")
    ...
    def _sync_chunked_user_titles(self, since_date: Optional[datetime], force_overwrite: bool, 
                                 dry_run: bool, max_records: Optional[int], 
                                 stats: Dict[str, Any]) -> Dict[str, Any]:
        """Process user titles data in chunks for large datasets"""
        ...
        while True:
            ...
            # Fetch chunk data
            chunk_data = self.client.get_chunked_user_titles(offset, current_chunk_size, since_date)
            ...
            # Process this chunk
            chunk_stats = self._process_user_titles_batch(chunk_data, force_overwrite, dry_run, stats)
    ...
    def _process_user_titles_batch(self, user_titles_data: list, force_overwrite: bool, 
                                  dry_run: bool, stats: Dict[str, Any]) -> Dict[str, Any]:
        """Process a batch of user titles data using bulk operations"""
        
        if not user_titles_data:
            return stats
            
        field_mapping = self.client.get_field_mapping()
        
        # Process in smaller batches for efficiency
        batch_count = (len(user_titles_data) + self.batch_size - 1) // self.batch_size
        logger.info(f"Processing {len(user_titles_data)} user titles in {batch_count} batches of {self.batch_size}")
        
        for i in range(0, len(user_titles_data), self.batch_size):
            batch_num = (i // self.batch_size) + 1
            batch_data = user_titles_data[i:i + self.batch_size]
            
            logger.info(f"Processing batch {batch_num}/{batch_count}: records {i+1}-{min(i+self.batch_size, len(user_titles_data))}")
