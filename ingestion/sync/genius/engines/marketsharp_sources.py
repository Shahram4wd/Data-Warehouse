...
class GeniusMarketsharpSourcesSyncEngine(GeniusBaseSyncEngine):
    """
    Sync engine for Genius MarketSharp source data following established patterns.
    Uses client → processor architecture without config dependencies.
    """
    
    def __init__(self):
        super().__init__('marketsharp_sources')
    ...
    def sync_marketsharp_sources(self, 
        ...
        try:
            return asyncio.run(self.sync_marketsharp_sources_async(**sync_params))
        except RuntimeError as e:
            if "asyncio.run() cannot be called from a running event loop" in str(e):
                # Already in an event loop, use different approach
                import asyncio
                loop = asyncio.get_event_loop()
                return loop.run_until_complete(self.sync_marketsharp_sources_async(**sync_params))
            else:
                raise
    ...
    async def execute_sync(self, 
        ...
        """Execute the marketsharp sources sync process - adapter for standard sync interface"""
        
        # Determine since_date based on full flag
        since_date = None if full else since
        
        return await self.sync_marketsharp_sources_async(
    ...
    def _bulk_upsert_records(self, validated_records: List[dict], force_overwrite: bool) -> Dict[str, int]:
        """Perform bulk upsert of marketsharp source records using modern bulk_create with update_conflicts"""
