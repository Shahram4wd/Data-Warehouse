...
class Command(BaseSyncCommand):
    help = 'Sync CallRail trackers data with standardized flags'
    crm_name = 'callrail'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not sync_result.get('success', True):
                raise CommandError('CallRail trackers sync failed')
    ...
    async def _run_sync(self, full_sync, force_overwrite, dry_run, max_records, **sync_params):
        ...
        sync_engine = TrackersSyncEngine(dry_run=dry_run, batch_size=batch_size)
    ...
    def output_results(self, result):
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS('✓ CallRail trackers sync completed successfully!')
            )
        else:
            self.stdout.write(
                self.style.ERROR('✗ CallRail trackers sync failed')
            )
        ...
        self.stdout.write(f"Trackers: {total_processed} processed ({total_created} created, {total_updated} updated, {total_errors} failed)")
