"""
Management command to sync CallReil companies data
"""
import logging
import asyncio
...
class Command(BaseCommand):
    help = 'Sync CallReil companies data'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Starting CallReil companies sync for account: {account_id}')
            )
            ...
            sync_result = asyncio.run(
                self._run_sync(account_id, full_sync, dry_run)
            )
            ...
            logger.error(f"CallReil companies sync failed: {e}")
            raise CommandError(f"Sync failed: {e}")
    
    async def _run_sync(self, account_id, full_sync, dry_run):
