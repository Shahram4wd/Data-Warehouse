...
class Command(BaseCommand):
    help = "Download job statuses directly from the database and update the local database."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="job_status",
            help="The name of the table to download data from. Defaults to 'job_status'."
        )
    ...
    def _process_batch(self, rows):
        ...
        to_create = []
        to_update = []
        existing_records = Genius_JobStatus.objects.in_bulk([row[0] for row in rows])

        for row in rows:
            try:
                # Extract fields from row
                record_id, label, is_system = row

                # Convert is_system to SmallIntegerField
                is_system = int(is_system) if is_system is not None else 0

                if record_id in existing_records:
                    # Update existing record
                    record_instance = existing_records[record_id]
                    record_instance.label = label
                    record_instance.is_system = is_system
                    to_update.append(record_instance)
                else:
                    # Create new record
                    to_create.append(Genius_JobStatus(
                        id=record_id,
                        label=label,
                        is_system=is_system
                    ))
        ...
        if to_create:
            Genius_JobStatus.objects.bulk_create(to_create, batch_size=BATCH_SIZE)
            self.stdout.write(self.style.SUCCESS(f"Created {len(to_create)} job status records"))
        
        if to_update:
            Genius_JobStatus.objects.bulk_update(
                to_update,
                ['label', 'is_system'],
                batch_size=BATCH_SIZE
            )
            self.stdout.write(self.style.SUCCESS(f"Updated {len(to_update)} job status records"))
