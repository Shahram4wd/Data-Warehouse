"""
Django management command for syncing Genius job statuses using the new sync engine architecture.
...
from ingestion.sync.genius.clients.job_statuses import GeniusJobStatusClient
from ingestion.sync.genius.processors.job_statuses import GeniusJobStatusProcessor
from ingestion.models.genius import Genius_JobStatus
...
class Command(BaseCommand):
    help = 'Sync Genius job statuses data using the standardized sync engine'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.exception("Genius job statuses sync failed")
            self.stdout.write(
    ...
    def execute_sync(self, **kwargs):
        ...
        logger.info(f"Starting job statuses sync (full={full}, dry_run={dry_run})")
        ...
        try:
            ...
            client = GeniusJobStatusClient()
            processor = GeniusJobStatusProcessor(Genius_JobStatus)
            ...
            logger.info("Fetching job statuses from Genius database...")
            raw_records = client.get_job_statuses(
                ...
                for i, record in enumerate(chunk_records, start=chunk_start):
                    try:
                        ...
                        if record_id:
                            ...
                            try:
                                if Genius_JobStatus.objects.filter(id=record_id).exists():
                                    # Update existing record
                                    obj, created = Genius_JobStatus.objects.get_or_create(
                                        id=record_id,
                                    ...
                                    # Create new record
                                    objects_to_create.append(Genius_JobStatus(**transformed_record))
                            except Exception as e:
            ...
            logger.info(f"Completed job statuses sync: {stats['processed']} processed, "
    ...
    def _process_batch(self, objects_to_create, objects_to_update, dry_run, debug):
        ...
        if objects_to_create:
            try:
                created_objects = Genius_JobStatus.objects.bulk_create(
                    objects_to_create, 
                    ignore_conflicts=True
                )
                batch_stats['created'] = len(created_objects)
                if debug:
                    logger.info(f"Created {len(created_objects)} new job status records")
                    
            except Exception as e:
                logger.error(f"Error creating job status records: {e}")
        ...
        if objects_to_update:
            try:
                ...
                if debug:
                    logger.info(f"Updated {batch_stats['updated']} existing job status records")
                    
            except Exception as e:
                logger.error(f"Error updating job status records: {e}")
