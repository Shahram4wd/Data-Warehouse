"""
Django management command for syncing Genius services using the new sync engine architecture.
...
from ingestion.sync.genius.engines.services import GeniusServicesSyncEngine

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = 'Sync Genius services data using the standardized sync engine'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.exception("Genius services sync failed")
            self.stdout.write(
    ...
    async def execute_async_sync(self, **kwargs):
        ...
        engine = GeniusServicesSyncEngine()
