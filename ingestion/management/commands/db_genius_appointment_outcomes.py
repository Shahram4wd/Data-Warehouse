"""
Django management command for syncing Genius appointment outcomes using the new sync engine architecture.
...
class Command(BaseCommand):
    help = 'Sync Genius appointment outcomes data using the standardized sync engine'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.exception("Genius appointment outcomes sync failed")
            self.stdout.write(
    ...
    def execute_sync(self, **kwargs):
        ...
        logger.info(f"Starting appointment outcomes sync (full={full}, dry_run={dry_run})")
        ...
        try:
                ...
                for i, record in enumerate(chunk_records, start=chunk_start):
                    try:
                        ...
                        if record_id:
                            ...
                            try:
                                if Genius_AppointmentOutcome.objects.filter(id=record_id).exists():
                                    # Update existing record
                                    obj, created = Genius_AppointmentOutcome.objects.get_or_create(
                                        id=record_id,
            ...
            logger.info(f"Completed appointment outcomes sync: {stats['processed']} processed, "
    ...
    def create_sync_record(self, configuration):
        ...
        return SyncHistory.objects.create(
            sync_type='genius_appointment_outcomes',
            status='running',
    ...
    def _process_batch(self, objects_to_create, objects_to_update, dry_run, debug):
        ...
        if objects_to_create:
            try:
                created_objects = Genius_AppointmentOutcome.objects.bulk_create(
                    objects_to_create, 
        ...
        if objects_to_update:
            try:
                ...
                if debug:
                    logger.info(f"Updated {batch_stats['updated']} existing appointment outcome records")
