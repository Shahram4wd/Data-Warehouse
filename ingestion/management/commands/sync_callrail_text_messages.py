"""
Management command to sync CallRail text messages data
"""
import logging
import asyncio
import os
...
class Command(BaseSyncCommand):
    help = 'Sync CallRail text messages data with standardized flags'
    crm_name = 'callrail'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--company-id',
            type=str,
            help='Optional company ID to filter text messages'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not sync_result.get('success', True):
                raise CommandError('CallRail text messages sync failed')
            
        except Exception as e:
            logger.error(f"CallRail text messages sync failed: {e}")
    ...
    async def _run_sync(self, full_sync, force_overwrite, dry_run, max_records, **sync_params):
        ...
        sync_engine = TextMessagesSyncEngine(dry_run=dry_run, batch_size=batch_size)
        ...
        return await sync_engine.sync_text_messages(
            full_sync=full_sync,
    ...
    def output_results(self, result):
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS('✓ CallRail text messages sync completed successfully!')
            )
        else:
            self.stdout.write(
                self.style.ERROR('✗ CallRail text messages sync failed')
            )
        ...
        self.stdout.write(f"Text messages: {total_processed} processed ({total_created} created, {total_updated} updated, {total_errors} failed)")
