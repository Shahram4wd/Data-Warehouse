"""
Management command to sync CallReil calls data
"""
import logging
import asyncio
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Starting CallReil calls sync for account: {account_id}')
            )
            ...
            sync_result = asyncio.run(
                self._run_sync(account_id, full_sync, dry_run, **sync_params)
            )
            ...
            logger.error(f"CallReil calls sync failed: {e}")
            raise CommandError(f"Sync failed: {e}")
    
    async def _run_sync(self, account_id, full_sync, dry_run, **sync_params):
