"""
Management command to sync CallRail tags (enterprise pattern)
"""
import logging
import asyncio
import os
...
class Command(BaseSyncCommand):
    help = 'Sync CallRail tags data with standardized flags'
    crm_name = 'callrail'
    entity_name = 'tags'
    ...
    def handle(self, *args, **options):
        try:
            ...
            if not quiet:
                self.stdout.write(self.style.SUCCESS('Starting CallRail tags sync...'))
                if dry_run:
            ...
            result = asyncio.run(self._run_sync(full_sync, force_overwrite, dry_run, max_records, batch_size, **sync_params))
            ...
            logger.error(f'CallRail tags sync failed: {e}')
            raise CommandError(f'Sync failed: {e}')

    async def _run_sync(self, full_sync, force_overwrite, dry_run, max_records, batch_size, **sync_params):
        engine = TagsSyncEngine(dry_run=dry_run, batch_size=batch_size)
        return await engine.sync_tags(full_sync=full_sync, force_overwrite=force_overwrite, max_records=max_records, **sync_params)

    def _output_results(self, result):
        success = not result.get('error')
        if success:
            self.stdout.write(self.style.SUCCESS('✓ CallRail tags sync completed successfully!'))
        else:
            self.stdout.write(self.style.ERROR('✗ CallRail tags sync failed'))

        self.stdout.write(
            f"Tags: {result.get('total_processed', 0)} processed "
