"""
Management command to sync CallRail tags data
"""
import logging
import asyncio
import os
...
class Command(BaseSyncCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--company-id',
            type=str,
            help='Optional company ID to filter tags'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not sync_result.get('success', True):
                raise CommandError('CallRail tags sync failed')
    ...
    async def _run_sync(self, full_sync, force_overwrite, dry_run, max_records, **sync_params):
        ...
        sync_engine = TagsSyncEngine(dry_run=dry_run, batch_size=batch_size)
        ...
        return await sync_engine.sync_tags(
            full_sync=full_sync,
    ...
    def output_results(self, result):
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS('✓ CallRail tags sync completed successfully!')
            )
        else:
            self.stdout.write(
                self.style.ERROR('✗ CallRail tags sync failed')
            )
        ...
        self.stdout.write(f"Tags: {total_processed} processed ({total_created} created, {total_updated} updated, {total_errors} failed)")
