...
Django management command to sync only LeadConduit events following
sync_crm_guide.md naming conventions.

Usage:
    python manage.py sync_leadconduit_events
    python manage.py sync_leadconduit_events --start-date 2024-01-01
    python manage.py sync_leadconduit_events --force
"""
import logging
...
from ingestion.sync.leadconduit.engines.base import LeadConduitEventsSyncEngine
...
class Command(BaseCommand):
    help = 'Sync LeadConduit events only'
    ...
    def handle(self, *args, **options):
        ...
        if not options['quiet']:
            self.stdout.write(
                self.style.SUCCESS('Starting LeadConduit events sync...')
            )
        ...
        try:
            ...
            sync_engine = LeadConduitEventsSyncEngine(config)
            ...
            if not result.get('success', False):
                raise CommandError('Events sync failed')
            
        except Exception as e:
            logger.error(f"Command failed: {e}")
            raise CommandError(f'Events sync failed: {e}')
    
    def run_sync(self,
                 sync_engine: LeadConduitEventsSyncEngine,
                 start_date=None,
                 end_date=None,
                 force=False,
        ...
        if not quiet:
            ...
            if date_range:
                self.stdout.write(f"Syncing events{date_range}")
            else:
                self.stdout.write("Syncing events with default date range")
    ...
    def output_results(self, result: Dict[str, Any]):
        ...
        if result.get('success'):
            self.stdout.write(
                self.style.SUCCESS('✓ LeadConduit events sync completed successfully')
            )
        else:
            self.stdout.write(
                self.style.ERROR('✗ LeadConduit events sync failed')
            )
