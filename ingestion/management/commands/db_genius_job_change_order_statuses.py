"""
Django management command for syncing Genius job change order statuses using the new sync engine architecture.
...
from ingestion.sync.genius.engines.job_change_order_statuses import GeniusJobChangeOrderStatusesSyncEngine

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = 'Sync Genius job change order statuses data using the standardized sync engine'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.exception("Genius job change order statuses sync failed")
            self.stdout.write(
    ...
    async def execute_async_sync(self, **kwargs):
        ...
        engine = GeniusJobChangeOrderStatusesSyncEngine()
