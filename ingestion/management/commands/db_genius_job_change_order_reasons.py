...
class Command(BaseCommand):
    help = "Download job change order reasons directly from the database and update the local database."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--table",
            type=str,
            default="job_change_order_reason",
            help="The name of the table to download data from. Defaults to 'job_change_order_reason'."
        )
    ...
    def _process_batch(self, rows):
        ...
        to_create = []
        to_update = []
        existing_records = Genius_JobChangeOrderReason.objects.in_bulk([row[0] for row in rows])

        for row in rows:
            try:
                # Extract fields from row
                record_id, label, description = row
                ...
                if record_id in existing_records:
                    ...
                    # Create new record
                    to_create.append(Genius_JobChangeOrderReason(
                        id=record_id,
                        label=label,
        ...
        if to_create:
            Genius_JobChangeOrderReason.objects.bulk_create(to_create, batch_size=BATCH_SIZE)
            self.stdout.write(self.style.SUCCESS(f"Created {len(to_create)} job change order reason records"))
        
        if to_update:
            Genius_JobChangeOrderReason.objects.bulk_update(
                to_update,
                ['label', 'description'],
                batch_size=BATCH_SIZE
            )
            self.stdout.write(self.style.SUCCESS(f"Updated {len(to_update)} job change order reason records"))
