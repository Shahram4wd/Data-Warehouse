"""
Django management command for syncing Genius appointment types using the new sync engine architecture.
...
from ingestion.sync.genius.processors.appointment_types import GeniusAppointmentTypeProcessor
from ingestion.models.genius import Genius_AppointmentType
...
class Command(BaseCommand):
    help = 'Sync Genius appointment types data using the standardized sync engine'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.exception("Genius appointment types sync failed")
            self.stdout.write(
    ...
    def execute_sync(self, **kwargs):
        ...
        logger.info(f"Starting appointment types sync (full={full}, dry_run={dry_run})")
        ...
        try:
            ...
            client = GeniusAppointmentTypeClient()
            processor = GeniusAppointmentTypeProcessor(Genius_AppointmentType)
            ...
            logger.info("Fetching appointment types from Genius database...")
            raw_records = client.get_appointment_types(
                ...
                for i, record in enumerate(chunk_records, start=chunk_start):
                    try:
                        ...
                        if record_id:
                            ...
                            try:
                                if Genius_AppointmentType.objects.filter(id=record_id).exists():
                                    # Update existing record
                                    obj, created = Genius_AppointmentType.objects.get_or_create(
                                        id=record_id,
                                    ...
                                    # Create new record
                                    objects_to_create.append(Genius_AppointmentType(**transformed_record))
                            except Exception as e:
            ...
            logger.info(f"Completed appointment types sync: {stats['processed']} processed, "
    ...
    def _process_batch(self, objects_to_create, objects_to_update, dry_run, debug):
        ...
        if objects_to_create:
            try:
                created_objects = Genius_AppointmentType.objects.bulk_create(
                    objects_to_create, 
                    ignore_conflicts=True
                )
                batch_stats['created'] = len(created_objects)
                if debug:
                    logger.info(f"Created {len(created_objects)} new appointment type records")
                    
            except Exception as e:
                logger.error(f"Error creating appointment type records: {e}")
        ...
        if objects_to_update:
            try:
                ...
                if debug:
                    logger.info(f"Updated {batch_stats['updated']} existing appointment type records")
                    
            except Exception as e:
                logger.error(f"Error updating appointment type records: {e}")
